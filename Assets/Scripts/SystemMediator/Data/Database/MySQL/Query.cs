namespace Data.Database.MySQL
{
    ...
    public partial class Query
    {
        ...
        private const string SERVER_URL = "http://45.50.73.43/pon/PHP/call.php";
        ...
        private IEnumerator Run(Table output, string function, params string[] vars)
        {
            ...
            {
                string[] line = result.text.Split(ROW_DELIMITER);
                if (line[0] == "MySQL: Success") // MySQL succeeded
                {
                    if (line[1] != "Query: Failed") // Query succeeded
                    {
                        ...
                        if (line[1] != "Query:  rows") // A pull, not push statement
                        {
                            ...
                            {
                                string[] rows = line[i + 3].Split(FIELD_DELIMITER); // segment is one row, field=value field=value

                                for (int j = 0; j < rows.Length; j++)
                                {
                                    string[] pair = rows[j].Split(INFO_DELIMITER); // pair[0] holds field name, pair[1] holds value
